...
export const TodoListItemContent = ({ todo }: Props) => {
  const [isHover, setIsHover] = useState<boolean>(false);

  const { showForm, saveTodoData, setIsEditMode } = useTodoFormStore();

  const toggleIsHover = (val: boolean) => setIsHover(val);

  const { deleteTodoMutation } = useTodoQuery(todo);
  ...
  return (
    ...
    >
      ...
      {isHover && (
        <Container>
          <Icon onClick={clickEditBtnHandler}>
            <svg
          ...
          <Icon onClick={clickDeleteBtnHandler}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="1em"
              viewBox="0 0 448 512"
